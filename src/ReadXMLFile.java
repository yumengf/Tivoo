	...
	public static void main(String[] args) {
      ...
      //go through the list of events, process each event
		for (int i = 0; i < list.size(); i++) { 
		   ...
		   Element node = (Element) list.get(i); //get each individual node of event
		   
		   Element startNode = node.getChild("start");
		   
		   String date = startNode.getChildText("year")+ startNode.getChildText("twodigitmonth") + startNode.getChildText("twodigitday");
		   ...
		   {
			   P myP = new P();
			   myP.appendChild(new Text(date + node.getChildText("summary")));
...
//		System.out.println( html.write() ); //better if can print to an HTML file...
		Writer w = new OutputStreamWriter(new FileOutputStream("output filtered Duke Calender.html"));
		w.write(html.write());
