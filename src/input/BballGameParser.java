...
public class BballGameParser implements Parser{
	private DateTimeFormatter myFormatter = DateTimeFormat.forPattern("M/d/yyyy hh:mm:ss aa");
	...
	public ArrayList<CalendarEvent> parse(Element rootNode) throws JDOMException, IOException{
		...
		for(Object e : list){
			...
			String date = event.getChildText("StartDate") + " " + event.getChildText("StartTime");
			DateTime startDateTime = myFormatter.parseDateTime(date);
			
			String enddate = event.getChildText("EndDate") + " " + event.getChildText("EndTime");
